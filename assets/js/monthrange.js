var MONTHS = ["Jan","Feb","Mar","Apr","Mei","Jun","Jul","Agu","Sep","Okt","Nop","Des"];
...
function paintMonths(){
  ...
  $('.mpr-calendar .mpr-month').css("background","");
    ...
    $('.mrp-monthdisplay .mrp-lowerMonth').html(MONTHS[startmonth - 1]);
    $('.mrp-monthdisplay .mrp-upperMonth').html(MONTHS[endmonth - 1]);
    $('input[name="peler"]').val(startMonth);
    $('input[name="peler2"]').val(endMonth);
    if(startyear == parseInt($('.mpr-calendar:first h5 span').html()))
      $('.mpr-calendar:first .mpr-selected:first').css("background","#212121");
    if(endyear == parseInt($('.mpr-calendar:last h5 span').html()))
      $('.mpr-calendar:last .mpr-selected:last').css("background","#212121");
  }
