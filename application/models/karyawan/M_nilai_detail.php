...
class M_nilai_detail extends CI_Model {
	...
	protected function det_nilai_setting(){
		$id_user = $this->session->userdata('id_user');
		$periode = $this->session->userdata('periode');

		$this->db->select($this->select_column_app)
				 ->from(self::$appraisal_table)
				 ->join('aspek', 'aspek.id_aspek = '.self::$appraisal_table.'.id_aspek')
				 ->join('jaspek', 'jaspek.id_jAspek = aspek.id_jAspek')
				 ->join('users', 'appraisal.id_user = users.id_user')
				 ->join('skor', 'skor.id_user = users.id_user');

		$where_id = 'appraisal.id_user = "'.$id_user.'"';
		$this->db->where($where_id);

		$where_per = 'skor.periode = "'.$periode.'"';
